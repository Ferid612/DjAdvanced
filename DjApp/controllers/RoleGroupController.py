...
def create_role(request):
    ...
    response = JsonResponse(
        {"message": "New role added successfully."}, status=200)
...
def create_roles(request):
    ...
    existing_roles = session.query(UserRole.name).filter(UserRole.name.in_(role_names)).union(
        session.query(EmployeeRole.name).filter(EmployeeRole.name.in_(role_names))).all()
    existing_role_names = {r[0] for r in existing_roles}

    # Create new roles
    model_classes = {'user_role': UserRole, 'employee_role': EmployeeRole}
    new_roles = [model_classes[role['role_type']](name=role['role_name'], description=role['role_description'])
                 for role in roles if role['role_name'] not in existing_role_names]
    ...
    return JsonResponse(
        {
            'message': 'Request succesfully finished.',
            'added_roles': added_roles,
            'existing_roles': list(existing_role_names),
        },
        status=200,
    )
...
def create_permission(request):
    ...
    return JsonResponse(
        {"message": "New permission added successfully."}, status=200
    )
...
def create_permissions(request):
    ...
    existing_permissions = session.query(Permission.name).filter(
        Permission.name.in_(permission_names)).all()
    existing_permission_names = {p[0] for p in existing_permissions}

    # Create new permissions
    new_permissions = [Permission(name=p['name'], description=p['description'])
                       for p in permissions if p['name'] not in existing_permission_names]
    ...
    return JsonResponse(
        {
            ...
            'existing_permissions': list(existing_permission_names),
        },
        status=200,
    )
...
def assign_permission_to_role(request):
    ...
    if role_type == "user_role":
        ...
        role_id = session.query(EmployeeRole).filter_by(
    ...
    permission_id = session.query(Permission).filter_by(
        name=permission_name).first().id
    ...
    # Return a success message
    response = JsonResponse(
        {"message": "New role-permission relation added successfully."}, status=200)
...
def assign_permissions_to_role(request):
    ...
    if role_type == 'user_role':
        ...
        return JsonResponse(
            {'answer': 'False', 'message': 'Role type is not correct.'},
            status=400,
        )
    ...
    if missing_permissions := set(permission_names) - {
    ...
    for permission_id in permission_ids:
        if (
            existing_role_permission := session.query(RolePermission)
            .filter_by(employee_role_id=role.id, permission_id=permission_id)
            .first()
        ):
            existing_permissions.append(
                existing_role_permission.permissions.name)
        else:
            role_permission = RolePermission(
                employee_role_id=role.id, permission_id=permission_id)
            session.add(role_permission)
            added_permissions.append(
                permissions[permission_ids.index(permission_id)].name)
...
def create_group(request):
    ...
    if group_type == "user_group":
        ...
        for group in groups:
            added_group = EmployeeGroup(
                name=group['name'], description=group['description'])
            session.add(added_group)

    else:
        return JsonResponse(
            {'answer': 'False', 'message': "Incorrect group_type name."},
            status=404,
        )
    return JsonResponse(
        {'answer': 'True', "message": "New groups added successfully."},
        status=200,
    )
...
def assign_user_to_group_role(request):
    ...
    if not (applied_user and assigned_role and assigned_group):
        return JsonResponse(
            {
                'answer': 'False',
                'message': 'No record found matching either username, role name or group name',
            },
            status=404,
        )
    ...
    return JsonResponse(
        {
            'answer': 'true',
            'message': 'Assiging new group and role to user is succesfully finshed.',
        },
        status=200,
    )
...
def assign_employee_to_group_role(request):
    ...
    if not (applied_employee and assigned_role and assigned_group):
        return JsonResponse(
            {
                'answer': 'False',
                'message': 'No record found matching either employee, role name or group name',
            },
            status=404,
        )
    ...
    return JsonResponse(
        {
            'answer': 'true',
            'message': 'Assiging new group and role to employee is succesfully finshed.',
        },
        status=200,
    )

...
def handle_expired_access_token(request, session):
    ...
    refresh_token = check_refresh_token(
        refresh_token, decoded_refresh_token, person, session)
...
def check_access_token(access_token: str, person_id: object) -> str:
    ...
    access_token_expiration_time = datetime.datetime.utcnow(
    ) + datetime.timedelta(minutes=ACCESS_TOKEN_EXPIRATION_TIME)
...
def check_refresh_token(refresh_token, decoded_refresh_token, person, session):
    ...
    refresh_token_expiration_time = datetime.datetime.utcnow() + \
